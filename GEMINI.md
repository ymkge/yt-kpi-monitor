...
## 6. 今後の拡張タスク (Next Steps)

現在のMVP（最小機能版）の完成を受け、以下の拡張を予定している。

1.  **YouTube Analytics API (OAuth) 連携**:
    - 公開情報だけでなく、内部ログ（視聴維持率等）に基づくより高度なAI分析を実現する。
2.  **エラー通知の強化**:
    - API Quota切れや認証エラー発生時に、Slackへ即時アラートを飛ばす仕組みの導入。
3.  **Looker Studio による可視化**:
    - BigQueryに蓄積されたデータを活用し、グラフィカルなダッシュボードを構築する。
4.  **マルチチャンネル対応**:
    - 複数のチャンネルIDを配列で管理し、一括で監視・レポートを生成する構成への拡張。

---

## 7. Gemini CLIへの指示出しテンプレート
