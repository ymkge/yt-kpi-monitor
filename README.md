...
- [ ] [改善]月次レポーティングのブラッシュアップ対応 #43
  - 修正点を洗い出したら記載する。
- [ ] [改善]YouTube KPI デイリーレポートのCTR改善を促す要素の追加 #46
  - 現状CTRの低さからチャンネルが低評価になりインプレッションが極小になっている
  - 日々のslack投稿しているデイリーレポートで直近14日以内にリリースされた動画のクリック率 (CTR)を記載している
    - モニタリングだけでなく、改善必要であるということを明確に可視化したい。例えば以下のような区分けを作りレポートに反映させる
      - CTRが5%以上: 優秀なサムネイルである
      - CTRが3%以上: 標準的なサムネイルになっている
      - CTRが3%未満: ターゲット層に上手く届いていない（要改善であることを明確にレポートを見て判断できるようにしたい）
        - これらの内容は議論して詰めていきたい。youtube音楽BGMでの平均的なCTR水準をベースにしたい 
- [ ] [改善]週次レポートの「Gemini AI 戦略アドバイス」の改善対応 #48
  - 「Gemini AI 戦略アドバイス」だと味気ないので、「クロの打改善アドバイス」というタイトルに変更する
  - 「4. 翌週に向けた具体的なアクション案」とあるが、週次レポートが出たる段階ですでに翌週になっているので、「4. 今後に向けた具体的なアクション案」に変更する
  - アドバイス内容に関して、現状再生数があまり伸びてないのはチャンネル登録者以外にはレコメンドがあまりされていない状態（youtubeからの評価が低い）の為、週間総再生数が1000を超えていなければ、その点を考慮したコメントにしてほしい
    - 現状の課題と目標: CTRが低くインプレッションが伸びきらないこと。CTRが上位の動画10本ほどで3%以上になるようにしたい。こうならないと再生数が伸びない為、チャンネル登録者数も増えない。